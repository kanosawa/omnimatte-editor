...
| `CASPER_BACKEND` | `"cogvideox"` | 使用するバックエンド。`"wan"`（Wan2.1-Fun-1.3B-InP）または `"cogvideox"`（CogVideoX-Fun-V1.5-5b-InP）。下の `CASPER_TRANSFORMER_PATH` / `CASPER_CONFIG_PATH` はこの値から自動選択される |
| `CASPER_TRANSFORMER_PATH` | バックエンドに応じて `wan2.1_fun_1.3b_casper.safetensors` または `cogvideox_5b_casper.safetensors` の絶対パス | Casper の重みファイル |
| `CASPER_CONFIG_PATH` | バックエンドに応じて `config/default_wan.py` または `config/default_cogvideox.py` | ベース config（リポジトリ相対） |
| `CASPER_FPS` | `8` | 出力 fps |
...
`vendor/gen-omnimatte-public/inference/{wan2.1_fun,cogvideox_fun}/predict_v2v.py` の `load_pipeline` / `run_inference` 相当を、**`absl.app.run` / `absl.flags` から切り離した import 可能関数として再実装する**。

`CASPER_BACKEND` で 2 つのバックエンドを切り替えられるように、ファイルを 4 つに分けている。

| ファイル | 役割 |
|---|---|
| `casper_server/pipeline.py` | ディスパッチャ。`CASPER_BACKEND` に応じて以下のどちらかから `build_default_config` / `load_pipeline` / `run_one_seq` を re-export する |
| `casper_server/pipeline_common.py` | バックエンド共通のヘルパ（`ensure_repo_on_path()`、`save_video_high_quality()`） |
| `casper_server/pipeline_wan.py` | Wan2.1-Fun-1.3B-InP 用の `load_pipeline` / `run_one_seq` |
| `casper_server/pipeline_cogvideox.py` | CogVideoX-Fun-V1.5-5b-InP 用の `load_pipeline` / `run_one_seq` |

両バックエンドが共通で公開する関数は以下:

| 関数 | 概要 |
|---|---|
| `build_default_config() -> ConfigDict` | `gen-omnimatte-public/config/default_*.py` を `ml_collections` で読み、本仕様の上書き値（`matting_mode=all_fg`、`fps=8`、`num_inference_steps=1`、`temporal_window_size=21`、`transformer_path=<重み>`）を当てる。`sample_size` はリクエスト毎に動的に決まるため、ここでは設定しない |
| `load_pipeline(cfg) -> (pipeline, vae, generator)` | `predict_v2v.load_pipeline` 相当をバックエンド別に再実装（`absl` 非依存） |
...
`predict_v2v.py` はモジュールトップで `absl.flags` をグローバル登録するため **本サーバや sidecar から import しない**。必要な関数は `pipeline_*.py` 内に移植する。

#### バックエンド差分まとめ

| 項目 | Wan | CogVideoX |
|---|---|---|
| Transformer | `WanTransformer3DModel` | `CogVideoXTransformer3DModel` |
| VAE | `AutoencoderKLWan` | `AutoencoderKLCogVideoX` |
| Text encoder | `WanT5EncoderModel` | `T5EncoderModel`（diffusers 標準） |
| CLIP image encoder | `CLIPModel`（あり） | なし |
| Pipeline | `WanFunInpaintPipeline` | `CogVideoXFunInpaintPipeline` |
| Scheduler | `FlowMatchEulerDiscreteScheduler`（手動構築） | `DDIMScheduler.from_pretrained(subfolder="scheduler")` 等 |
| `pipeline()` 追加引数 | `clip_image=...` | `strength`, `use_vae_mask`, `stack_mask` |
