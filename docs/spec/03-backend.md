...
参照実装: [SAM2 公式リポジトリの `examples/segment_video_server.py`](https://github.com/facebookresearch/sam2/blob/main/examples/segment_video_server.py)
...
├── holder.py              # Casper（loading/ready/failed）
├── pipeline.py            # fork 済 predict_v2v.load_pipeline を importlib 経由で直接呼ぶ薄いラッパ + 本プロジェクト固有の run_one_seq を独自実装
...
`main.py` の FastAPI lifespan で `asyncio.create_task(sam2.load())` を呼ぶ。
...
5. `sam2.reset_state(state)` で前回結果をクリア
6. **bbox プロンプトを登録**（`sam2.add_bbox_prompt`）: video predictor の `add_new_points_or_box(..., box=bbox)` に bbox（pixel xyxy）を直接渡す。SAM2 ネイティブの box prompt embedding が memory bank に正規格納され、propagation の memory attention が想定通りに働く
...
```mermaid
flowchart TB
    A[base_video_path 確定] --> B["detectron2.wait_ready(30s)"]
...
lifespan で `asyncio.create_task(casper.load())` で非同期ロード開始。`load()` 内では `asyncio.to_thread(self._load_sync)` で `pipeline.load_pipeline(...)` を worker thread に逃がす。
...
| `load_pipeline(cfg) -> (pipeline, vae, generator)` | fork 済 `predict_v2v.load_pipeline` を `importlib.util` でファイルパスから直接ロードして呼ぶ。ディレクトリ名 `wan2.1_fun` にドットが含まれ通常の `from ... import` 構文が使えないため、`importlib` 経由でモジュールを生成し結果をキャッシュする |
| `run_one_seq(cfg, pipeline, vae, generator, seq_dir, save_dir, sample_size) -> str` | 1 シーケンス向けに独自実装。`load_pipeline` で得た pipeline を使い、`videox_fun.utils.utils.get_video_mask_input` で入力を作って推論。出力 mp4 のパスを返す。`sample_size` は `(H, W)` の int タプルで、両次元 16 の倍数必須。出力 mp4 は H.264 / yuv420p / crf 15（vendor 側の `save_videos_grid` がデフォルト品質で書き出すのを避け、本仕様で再実装） |

fork 版 `predict_v2v.py` は `absl` 系（`from absl import app, flags` / `flags.FLAGS` / `config_flags.DEFINE_config_file`）が `if __name__ == "__main__":` ブロックに閉じ込められているため、**import しても `flags.FLAGS` のグローバル登録は走らず副作用なしで取り込める**。`cfg.video_model.config_path` / `cfg.video_model.model_name` も `default_wan.get_config()` で `__file__` 相対の絶対パスに解決済みなので、`os.chdir` も不要。
...
- [ ] lifespan で `Casper.load()` が非同期で走り、`/health` がロード状態を返す
- [ ] `pipeline.py` が fork 済 `predict_v2v.py` を `importlib` 経由で副作用なし（`absl.flags` のグローバル登録が走らない）に取り込めている
